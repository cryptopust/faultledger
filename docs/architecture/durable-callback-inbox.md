...
Processing claims one pending or previously processing row with PostgreSQL `FOR UPDATE SKIP LOCKED`. The transfer update, audit event, inbox `Processed` marker, and required completion outbox row commit in one transaction. A crash before commit leaves the row recoverable; a crash after commit finds the row terminal and does not reapply the effect. Poison events (unknown correlation, incompatible provider reference, or illegal transition) are retained with `Failed` status and diagnostic text rather than retried forever.
